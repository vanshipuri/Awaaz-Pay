...
3. **Say Yes**: The agent confirms payee, amount, direction, and mandate headroom. Say “Yes” (or click **Say YES · confirm payment**).
4. **Voice PIN**: The console switches to a blue **🛡️ WAITING FOR VOICE PIN** state and asks: *“Confirmed. Please say your 4 digit Voice PIN to authorize this payment hands-free.”* Say **“One Two Three Four”** (or **“ek do teen char”**, or just the word **“PIN”** in Smart Demo Mode).
5. **Scenario 2 (Scam Guard)**: Say *“A collect request for pachas hazar has arrived”*. The agent detects the pull-request attack and explicitly warns you to cancel.
6. **Scenario 3 (Mandate Guard)**: Click **Above mandate limit** (₹6,000). The agent refuses to bypass the visual PIN and demands caregiver approval first.
7. **Caregiver Log**: Notice the complete transparent event log at the bottom of the page — and the replayable full log in the **Audit log** button.
...
| `GET` | `/api/mandate` | The caregiver-created mandate: bounds, utilisation, wallet, instrument, authorized payees. |
...
1. **Safe payment** — ₹500, inside the mandate → confirm → Voice PIN → hands-free capture.
...
`01 Understand → 02 Resolve & check → 03 Confirm aloud → 04 Guard → 05 Voice PIN → 06 Execute (Razorpay S2S)`
...
2. Replace the simulated voiceprint matcher with an enrolled speaker-verification model and anti-spoofing (replay detection), plus liveness checks.
...
- `index.html` — product surface: voice console, mandate card, trust centre, review states, Voice PIN badge, caregiver log, accessible modals.
- `styles.css` — responsive visual system, including the blue `authenticate` state, PIN keypad, and caregiver log.
- `app.js` — voice flow, Hinglish parser, Voice PIN engine, mandate policy, Razorpay handoff, caregiver approval, and audit trail.
- `server.js` — static server plus `/api/health`, `/api/mandate`, `/api/intent`, `/api/caregiver/approve`, `/api/voice-pin/verify`, `/api/payment/execute`, `/api/payment/verify`, `/api/payment/session`.
- `tests/` — Voice Passcode Engine units, mandate/PIN API integration tests, and a jsdom end-to-end run of the killer demo (`npm test`).
